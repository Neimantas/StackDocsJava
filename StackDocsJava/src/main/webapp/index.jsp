          ...
          <div class="col">
            <ul class="pagination">
              <c:if test="${numberOfPages>3 && pageNumber>2}">
                <li onclick="setPage(1);" class="page-item">
                  <a class="page-link text-dark" href="#" tabindex="-1">1</a>
                </li>
              </c:if>
              <c:if test="${pageNumber>3 && numberOfPages!=4}">
                <li onclick="setPage(${pageNumber!=numberOfPages?(pageNumber-3>1?pageNumber-3:1):pageNumber-4});" class="page-item">
              ...
              <c:if test="${numberOfPages>1}">
                <c:set var="pageBtn1" value="${pageNumber==1?pageNumber:(pageNumber==numberOfPages&&numberOfPages!=2?pageNumber-2:pageNumber-1)}"
                />
                <li onclick="setPage(${pageBtn1==pageNumber?0:pageBtn1});" class="page-item ${pageBtn1==pageNumber?'active disabled':''}">
                  <a class="page-link text-dark" href="#">${pageBtn1}</a>
                </li>
              </c:if>
              <c:if test="${numberOfPages>1}">
                <c:set var="pageBtn2" value="${pageNumber==1?pageNumber+1:(pageNumber==numberOfPages&&numberOfPages!=2?pageNumber-1:pageNumber)}"
                />
                <li onclick="setPage(${pageBtn2==pageNumber?0:pageBtn2});" class="page-item ${pageBtn2==pageNumber?'active disabled':''}">
                  <a class="page-link text-dark" href="#">${pageBtn2}</a>
                </li>
              </c:if>
              <c:if test="${numberOfPages>2}">
                <c:set var="pageBtn3" value="${pageNumber==1?pageNumber+2:(pageNumber==numberOfPages?pageNumber:pageNumber+1)}" />
                <li onclick="setPage(${pageBtn3==pageNumber?0:pageBtn3});" class="page-item ${pageBtn3==pageNumber?'active disabled':''}">
                  <a class="page-link text-dark" href="#">${pageBtn3}</a>
                </li>
              </c:if>
              <c:if test="${(numberOfPages>4) && (pageNumber<numberOfPages-2)}">
                <li onclick="setPage(${pageNumber!=1?(pageNumber+3<numberOfPages?pageNumber+3:numberOfPages):pageNumber+4});" class="page-item">
